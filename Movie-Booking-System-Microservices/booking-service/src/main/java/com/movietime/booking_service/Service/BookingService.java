...
public class BookingService {
    ...
    public BookingResponse createBooking(CreateBookingRequest req, String userId) {
        ...
        //     String key = "lock:session:" + sessionId + ":seat:" + seat.getSeatId();
...
// Track successfully locked seats
    ...
    public BookingResponse confirmPayment(Long bookingId, String paymentId) {
        ...
        for (BookingSeat seat : seats) {
            ...
            String key = "lock:session:" + booking.getSessionId() + ":seat:" + seat.getSeatId();
            redisTemplate.delete(key);
        }
        publishBookingConfirmed(booking);
        log.info("BOOKING_CONFIRMED | bookingId={} | userId={} | sessionId={}", booking.getId(), booking.getUserId(), booking.getSessionId());
        return BookingResponse.builder()
            .id(booking.getId())
            .sessionId(booking.getSessionId())
            .userId(booking.getUserId())
    ...
    public List<Long> getBookedSeats(Long sessionId) {
        return bookedSeatRepository.findBySessionId(sessionId)
                .stream()
                .map(BookedSeat::getSeatId)
                .toList();
    }
    public SeatStatusResponse getSeatStatus(Long sessionId) {
        ...
        Set<String> lockedKeys = redisTemplate.keys("lock:session:" + sessionId + ":seat:*");
    ...
    public void publishBookingConfirmed(Booking booking) {
        ...
        payload.put("sessionId", booking.getSessionId());
